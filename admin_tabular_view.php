...
try {
    switch ($tab) {
        case 'withdrawals':
            ...
            $sql = "SELECT w.id, u.username, w.amount, w.status, w.requested_at, w.processed_at FROM withdrawals w JOIN users u ON w.user_id = u.id ORDER BY {$sort_column} {$sort_order_sql}";
        ...
        case 'loans':
            ...
            $sort_column = in_array($sort_column, $valid_columns) ? $sort_column : 'id';
            $sql = "SELECT l.id, u.username, l.amount, l.balance, l.status, l.requested_at, l.approved_at FROM loans l JOIN users u ON l.user_id = u.id ORDER BY {$sort_column} {$sort_order_sql}";
        ...
        default:
            ...
            $sort_column = in_array($sort_column, $valid_columns) ? $sort_column : 'id';
            $sql = "SELECT s.id, u.username, s.amount, s.created_at FROM savings s JOIN users u ON s.user_id = u.id ORDER BY {$sort_column} {$sort_order_sql}";
