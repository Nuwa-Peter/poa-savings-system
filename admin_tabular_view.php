...
try {
    switch ($tab) {
        case 'withdrawals':
            ...
            $sql = "SELECT w.*, u.username FROM withdrawals w JOIN users u ON w.user_id = u.id ORDER BY {$sort_column} {$sort_order_sql}";
        ...
        case 'loans':
            ...
            $sort_column = in_array($sort_column, $valid_columns) ? $sort_column : 'id';
            $sql = "SELECT l.*, u.username FROM loans l JOIN users u ON l.user_id = u.id ORDER BY {$sort_column} {$sort_order_sql}";
...
<div class="container mx-auto mt-10 p-4">
    <h1 class="text-3xl font-bold text-gray-900 dark:text-gray-100 mb-6">Tabular Data View</h1>
